  ...
  <div :class="['absolute', 'top-0', 'right-0',clicked ? 'sm:w-3/5 w-full':'','bg-black','z-20']">
    ...
    <div class="relative border-2 border-green-400 pb-6" v-if="clicked">
      ...
      <emailAnimation class="text-white h-full w-64 mx-auto" :state.sync="Email_state">
        <template v-slot:loading_text><h1 class="text-white font-agncy text-center text-2xl">sending Your Email...</h1></template>
        <template v-slot:done_text><h1 class="text-white font-agncy text-center text-2xl">Thank You For Reaching Out.</h1></template>
      </emailAnimation>
    </div>
  </div>
</template>
